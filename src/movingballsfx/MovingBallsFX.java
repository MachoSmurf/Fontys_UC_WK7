...
public class MovingBallsFX extends Application {
    ...
    private Ball[] ballArray = new Ball[10];    // array containing balls.
    private Thread[] threadArray = new Thread[10]; // array containing threads, so you can interrupt them
    private CheckBox[] checkBoxArray = new CheckBox[10];  // to control creating and interrupting ball threads.
    private Circle[] circleArray = new Circle[10]; // array containing circles, representing balls
    ...
    private Monitor ballMonitor;
    
    // Changed by Nico Kuijpers
    // Create reader-writer monitor
    RWMonitorInterface RW;
    ...
    private void checkBoxMouseClicked(MouseEvent event, int index) {
        ...
        if (cb.isSelected() && index < 5) { 
            ...
            Ball b = new Ball(minX, maxX, minCsX, maxCsX, y, Role.READER);
            ballArray[index] = b;
            // Changed by Nico Kuijpers
            ...
            Ball b = new Ball(minX, maxX, minCsX, maxCsX, y, Role.WRITER);
            ballArray[index] = b;
            // Changed by Nico Kuijpers
            // Reference to reader-writer monitor
            Thread t = new Thread(new BallRunnable(b, ballMonitor));
    ...
    /**
     * use BallArray to cycle through Balls, find corresponding circles 
     * to draw in circleArray.
     */
